...
## Non-Linear Sector/Deployment Selection (2026-09-22)

Still the same session. Matthew pushed back that he blows through Sectors 1-3 quickly and then hits Sector 4 at under 30% Domain II mastery, forcing him to grind the one sector with the least content by design (its year window is only 2 years vs. 3 for the others). He proposed either enriching content further, or rethinking the 4-sector structure altogether.

**Quantified the actual mismatch before proposing anything**, via a standalone Node harness simulating full careers at different skill levels: **every single simulated career reached Sector 4 (the cap) after exactly 3 successful runs**, regardless of skill, since sector advancement only ever required a checkpoint clear — nothing to do with mastery. Reaching mastery-ready took 10-55 more runs depending on skill (worse at lower success rates). That means **a player spends 80-95%+ of total playtime already capped in whichever sector is "last"** — true no matter how many sectors exist or how early ones are stocked, since they're each visited exactly once before advancing. This reframed the three options on the table: enriching Sectors 1-3 has near-zero leverage (barely visited), enriching all sectors evenly is partly wasted effort for the same reason, and more/narrower sectors alone just relocates the camping problem to sector N. Matthew chose the deeper fix: decouple deployment year-selection from linear sector advancement entirely, picking non-linearly ("bounce around in time") so every run draws from the full cross-window pool instead of whichever slice a counter happened to be parked on. Confirmed via `AskUserQuestion`: weight the pick toward least-recently-visited windows (not pure random, not player-chosen), and in the Hub/map UI show the actual year range instead of a "SECTOR N" label — but hold off on showing the *location* name too, since the current site pool (`SITE_POOL_BY_DOMAIN` in `build_nodes.py`) only has 3-4 real named facilities per domain, not enough to avoid feeling repetitive at the Hub-preview level.

**Change (`game/web/mapgen.js`):** Removed the `sectorYearWindow(sectorIndex)` linear lookup. Added `pickDeploymentWindow(profile, rng)`, which weights each of the 4 windows by `staleness²` where staleness is `deployments_completed - window_last_visited[i]` (a never-visited window defaults to maximally stale) — strong bias toward long-unvisited windows without being a strict, predictable round-robin. `generateSectorMap()` now takes an explicit `windowIndex` param (falls back to picking its own if omitted, for ad hoc/test callers) instead of deriving the window from `profile.sector_index`, and returns the resolved `windowIndex` so the caller can record the visit.

**Change (`game/web/app.js`):** Replaced `profile.sector_index` with `window_last_visited` (per-window last-visited-at-deployment-count array), `deployments_completed` (counter), and `pending_window_index` (the already-picked window for the *next* deployment, computed once at Hub arrival via `showHub()` so the preview line and the eventual generated map always agree — it's consumed and cleared when "Begin Deployment" is actually clicked, not re-rolled on every Hub re-render). `endRun()` still sets `sector_cleared_at_current_rank` on success for rank-gating (untouched) but no longer advances any sector counter. Hub's deployment-preview line changed from `NEXT DEPLOYMENT — SECTOR N · {year}-{year}` to `NEXT DEPLOYMENT — {year}-{year}`; the map screen's eyebrow label changed from `SECTOR N` to just the deployment's actual year range.

**Verified via a Node harness (8 seeded careers):** window-visit distribution came back fairly even across all 4 windows (e.g. 6/9/8/9 visits over one ~35-run career) instead of 100% parked on window 4 as before. Total runs-to-mastery-ready came back roughly comparable to before (the total amount of correct-answer volume needed didn't change) but is now spread across the full pool instead of concentrated on the thinnest slice.

**Verified via Playwright:** Hub preview and map eyebrow both show only the year range with no "SECTOR" text, confirmed across 8 consecutive simulated deployments with zero console errors; `windowIndex` sequence across those 8 showed real non-linear bouncing (`0,1,3,3,2,0,1,3`) hitting all 4 windows, not a fixed cycle. Separately confirmed rank-up still fires correctly end-to-end (seeded mastery + a real successful run + the actual RETURN TO HUB click promoted Junior Sim Tech → Operations Specialist and unlocked the Moulage Artist) — this touched `endRun()`, so it was worth re-checking explicitly rather than assuming the untouched line still worked.
